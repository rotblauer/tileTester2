...
updump: ## updump dumps the points collected in tracks.db to master.json.gz
	...
	dumpy --in=${TRACKS_DATA}/tracks.db --out=${TRACKS_DATA}/master.snip # Run the last dump from bolt(tracks.db) -> "out.json.gz"
	mv ${TRACKS_DATA}/master.snip.json.gz ${TRACKS_DATA}/master.json.gz
